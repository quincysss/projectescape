...
> 文档目标：统一物品、材料、装备、消耗品、研究、制作、配方可用条件、效果等静态数据的表格规范，确保后续新增、删改内容时优先修改 `.tab` 配置表，而不是修改脚本逻辑。
...
| required_conditions | string | 前置配方可用条件 |
...
所有制作、前哨修复等非研发表的 required_items 中的 item_id 必须存在于 items.tab；research.tab.required_items 必须留空或由兼容层忽略。
