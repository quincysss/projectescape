...
> 目标：整理主菜单、玩家档案、剧情、章节目标、序章周转物资和首次开店教学的工程落地方式。
...
systems/flow/prologue_return_flow_service.gd

systems/base/base_feature_unlock_service.gd
systems/base/starter_supply_service.gd
...
systems/chapters/first_shop_tutorial_service.gd
...
data/starter_supply/prologue_shop_starter_pack.json
...
- `dialogue_id`：对白段落 ID，例如 `world_intro_dialogue`、`first_departure_outpost_dialogue`、`first_return_success_dialogue`、`first_return_failed_dialogue`。
...
`prologue_shop_starter_pack.json` 示例：

```json
{
  "pack_id": "prologue_shop_starter_pack",
  "grant_once_profile_flag": "starter_shop_supply_granted",
  "items": [
    { "category": "c_basic_metal", "item_id": "raw_scrap_metal", "count": 2 },
    { "category": "c_basic_fabric", "item_id": "raw_dirty_cloth", "count": 2 },
    { "category": "c_basic_plastic", "item_id": "raw_plastic_piece", "count": 1 },
    { "category": "c_basic_electric", "item_id": "raw_old_coil", "count": 1 }
  ],
  "notes": "具体 item_id 以 items.tab 重构后的真实配置为准；本包只给原材料，不给可售物资。"
}
```
...
```json
{
  ...
  "objectives": [
    { "objective_id": "craft_first_sale_good", "label": "制造第一批可售物资" },
    { "objective_id": "shelve_first_sale_good", "label": "将可售物资摆上货台" },
    { "objective_id": "finish_first_shop_day", "label": "完成第一次开店结算" }
  ]
}
```
...
- 保存档案。
- 更新剧情 flag。
- 迁移旧档字段，补齐 `starter_shop_supply_granted`、`first_shop_tutorial_completed`、`chapter_1_goal_active`。
...
- 支持跳过。
- 播放完成回调。
- 分别处理 `world_intro_dialogue`、`first_departure_outpost_dialogue`、`first_return_success_dialogue`、`first_return_failed_dialogue`。
...
- 管理局外功能入口解锁状态。
- 新档默认 `shop_loop_unlocked=false`、`research_station_unlocked=false`。
- 第一段“回到404哨所与背景故事”对白完成后，只开放序章夜晚出发入口和基础仓库。
- 序章探索返回剧情播放完成后，设置 `shop_loop_unlocked=true` 和 `research_station_unlocked=true`。
- 通知局外昼夜阶段入口刷新锁定/可点击状态。
- 旧档缺少 shop_loop/research 解锁字段时，如果 `first_return_dialogue_seen==true`，读取时补齐为已解锁。

PrologueReturnFlowService：

- 接收序章结算结果。
- 判断返回剧情分支：成功撤离使用 `first_return_success_dialogue`，失败或空包使用 `first_return_failed_dialogue`。
- 在返回哨所 loading 完成且玩家按任意按钮后播放剧情。
- 剧情结束后调用 `StarterSupplyService`、`BaseFeatureUnlockService` 和 `ChapterProgressService`。

StarterSupplyService：

- 读取 `data/starter_supply/prologue_shop_starter_pack.json`。
- 校验 `starter_shop_supply_granted`，确保只发一次。
- 将周转物资写入仓库。
- 与 ProfileService/SaveService 原子化保存。
- 不直接发放可售物资。
...
- 监听货台上架事件。
- 监听开店结算完成事件。
- 三步完成后设置 `first_shop_tutorial_completed=true`、`chapter_1_completed=true`。
...
signal starter_supply_granted(pack_id)
signal chapter_goal_updated(chapter_id, objective_id, completed)
signal chapter_completed(chapter_id, surface_day)
signal first_shop_tutorial_completed(chapter_id)
...
2. 建立 ProfileService、SaveStorageAdapter、BaseFeatureUnlockService、StarterSupplyService、PrologueReturnFlowService、DialogueService、DialogueSpeakerRegistry、DialoguePortraitController、ChapterProgressService、FirstShopTutorialService、RunLoadingController。
3. 玩家档案保存 username、surface_day、intro_cinematic_seen、world_intro_dialogue_seen、first_departure_outpost_dialogue_seen、second_day_black_tide_reveal_seen、first_return_dialogue_seen、shop_loop_unlocked、research_station_unlocked、starter_shop_supply_granted、first_shop_tutorial_completed、chapter_1_goal_active、chapter_1_completed。
...
6. world_intro_dialogue 完成后进入序章夜晚出发入口；此时不开放白天经营主流程。
...
13. 序章首次探索结算后，无论成功撤离或失败撤离，返回哨所 loading 完成并按任意按钮后都要播放序章返回剧情；成功撤离和失败/空包使用不同 dialogue_id。
14. 序章返回剧情结束后调用 StarterSupplyService 发放一次性开店周转物资，开启正式白天经营循环和研究所，并激活第一章目标。
15. 第一章目标是重启杂货店：制造第一批可售物资、上架货台、完成第一次开店结算。
16. 完成首次开店结算后设置 first_shop_tutorial_completed=true、chapter_1_completed=true。
17. 结算界面点击返回哨所后进入 RETURN_TO_BASE loading，100% 后显示“按下任意按钮继续”，玩家按任意按钮后才回局外。
18. RETURN_TO_BASE loading 不增加 surface_day，不提交出发装备，不启用局内控制。
19. 不实现旧独立交易页签，不实现固定矿币解锁制造所，不实现额外制造设施作为第一章目标。
```

## 9. 验收标准

- 新档创建、保存、重启读取正常。
- 开场视频、回到404哨所与背景故事对白、首次出发任务对白、序章返回剧情只触发一次。
- 新档序章只进入夜晚出发入口；序章返回剧情结束后开启正式白天经营循环和研究所。
- 序章成功撤离和失败撤离都能触发返回剧情。
- 序章失败且未带回物资时，仍能通过周转物资完成首次制造和开店。
- 周转物资只发一次。
...
- 第一章目标能显示制造、上架、首次营业结算三步进度。
- 文档和 UI 不出现旧独立交易页签、额外制造设施章节目标或固定矿币解锁目标。
...
Feature: 25-05 Godot 模块与数据配置
  作为 开发者
  我希望工程模块按最新局外经营流程拆分
  以便后续不再把剧情、周转物资和开店教学堆在单一 base_scene 脚本中

  Scenario: PrologueReturnFlowService 负责序章返回兜底
    Given 玩家完成序章首次探索
    And 返回哨所 loading 已完成
    When 玩家按任意按钮回到局外
    Then PrologueReturnFlowService 根据结算结果选择返回剧情
    And StarterSupplyService 发放一次性开店周转物资
    And ChapterProgressService 激活第一章首次营业目标
```
