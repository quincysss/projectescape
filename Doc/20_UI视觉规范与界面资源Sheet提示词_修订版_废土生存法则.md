...
描边不能太厚，避免遮挡美术。
描边颜色要和普通霓虹点缀区分。
前哨材料或特殊事件物接近消失时可轻微闪烁；普通容器不做倒计时闪烁。
...
容器格子与背包格子复用。
容器标题条可根据容器类型变色。
普通容器不显示倒计时条；危险提示只通过场景事件、前哨材料或异常预兆表现。
```
...
```text
功能入口数量会扩展，所以右侧必须预留空间。
当前选中入口有高亮描边。
未解锁入口可以锁定态显示。
未开发完成入口必须置灰并禁止点击。
V0.2 白天准备入口顺序为：仓库、制造所、研究所、图鉴。
顶部或右上角显示当前矿币数量。
```
...
```text
货台界面只在白天营业阶段开放。
白天准备阶段可以查看当日需求榜，但不能开始成交。
未选择可售物资时上架按钮禁用。
非 sale_good 物品不显示在可上架列表。
货台已满时上架按钮禁用。
成交成功后刷新货台数量、仓库数量和矿币显示。
物品图标 hover 时显示局外道具详情 Tooltip；Tooltip 可显示预估基础成交价，不显示本日总收益。
```
...
S 级道具可以在名称左侧小点、图标阴影或小标签上体现高稀有度。
不要把描述文字烘焙进 PNG。
不要做高亮商业卡牌风格。
```
...
Elements: research node locked, research node available, research node completed, research connection line, mine coin cost badge, prerequisite badge, recipe card frame, crafting output frame, crafting material requirement badge, missing material warning badge, craft button active, craft button disabled, progress strip, completed stamp without text, small tool icon frame, small circuit icon frame.
...
```text
Create a transparent PNG UI sprite sheet for shop shelf, demand ranking, and currency interface elements in a 2D wasteland extraction game.
Grid: 5x5, one element per cell, no labels.
...
Elements: mine coin icon, small currency badge frame, shelf slot empty, shelf slot occupied, shelf slot locked, demand rank row frame, demand rank highlight badge, put-on-shelf button active, put-on-shelf button disabled, open-shop button frame, settlement total strip, sold success stamp without text, warning badge, small deal arrow icon, top currency bar frame.
...
局外界面先做总体框架，再拆仓库、制造、研究、图鉴、货台的细节控件。
```
